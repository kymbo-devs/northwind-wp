...
if( ! ( class_exists( 'woocommerce' ) && is_woocommerce() ) ){
    if( get_post_meta(  moore_get_current_id() ,'moore_met_show_breadcrumbs', true ) != 'no' ){
    		...
    		$html = '<div id="breadcrumbs">';
				$html .= '<ul class="breadcrumb">';
			        ...
			        $html .= '<li><a href="' . home_url('/') . '" title="'.esc_attr__('Home', 'moore').'">' . esc_attr__('Home', 'moore') . '</a></li> ';
			        ...
			        if ( is_category() ) {
				        ...
				        $html .= $separator.'<li>'.esc_html__('Archive by date', 'moore').' '.get_the_time('d').'</li>';

			        } elseif ( is_month() ) {

				        $html .= $separator.'<li><a href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a></li>';
				        $html .= $separator.'<li>' . esc_html__('Archive by month', 'moore').' ' . get_the_time('F') . '</li>';

			        } elseif ( is_year() ) {

			        	$html .= $separator.'<li>' . esc_html__('Archive by year', 'moore').' ' . get_the_time('Y') . '</li>';
			            ...
			            $html .=  $separator.'<li>' . esc_html__('Search results for', 'moore').' ' . get_search_query() . '</li>';
			        	...
			        	$html .= $separator.'<li>' . esc_html__('Archive by tag', 'moore').' ' . single_tag_title('', false) . '</li>';

			        } elseif ( is_author() ) {

			        	global $author;
			        	$userdata = get_userdata($author);
			        	$html .= $separator.'<li>' . esc_html__('Articles posted by', 'moore').' ' . $userdata->display_name . '</li>';

			        } elseif ( is_home() ){

			        	$html .= $separator.'<li>' . esc_html__('Blog', 'moore').'&nbsp;' . '</li>';

			        }elseif ( is_404() ) {

			        	$html .= $separator.'<li>' . esc_html__('Page not found', 'moore') . '</li>';
